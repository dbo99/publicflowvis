...
pineflat_fnf <- rbind(pineflat_fnf, pineflat_fnf_change) %>% mutate(value = as.double(value))
}
}
pineflat_fnf_af <- pineflat_fnf %>% mutate(value = value * 1.98347, value = round(value,1)) %>% mutate( unit = "af")
pineflat_fnf <- rbind(pineflat_fnf, pineflat_fnf_af)

pineflat_fnf <- data.frame(pineflat_fnf) %>% mutate(value = unlist(value))  %>% filter(timeseq != "nextlatest") %>% select(-source) %>% 
                                            rename_all(paste0, "_corps") %>% 
