...
cdec_res_table  <- cdec_res_table %>% rename("res" = !!names(.[1]),
                                             "cdecid" = !!names(.[2]),
                                             "totalcapacity_cdec_NA_instant_af_date" = !!names(.[3]),            # totalcapacity_cdec_NA_instant_af_date
                                             ...
                                             "histstorvol_cdec_NA_histmean_af_date" = !!names(.[8]),          # histstorvol_cdec_NA_histmean_af_date
                                             "histstorval_cdec_NA_histmean_percent_date" = !!names(.[9])  ,    # histstorval_cdec_NA_histmean_percent_date                                     
                                             "outflow_cdec_latest_meandly_af_date" = !!names(.[10]),            # outflow_cdec_latest_meandly_af_date
                                             "inflow_cdec_latest_meandly_af_date" = !!names(.[11]),             # inflow_cdec_latest_meandly_af_date
...
cdec_res_table$totalcapacity_cdec_NA_instant_af_date <- gsub(",", "", cdec_res_table$totalcapacity_cdec_NA_instant_af_date)
...
cdec_res_table$histstorvol_cdec_NA_histmean_af_date <- gsub(",", "", cdec_res_table$histstorvol_cdec_NA_histmean_af_date)
cdec_res_table$outflow_cdec_latest_meandly_af_date <- gsub(",", "", cdec_res_table$outflow_cdec_latest_meandly_af_date)
cdec_res_table$inflow_cdec_latest_meandly_af_date <- gsub(",", "", cdec_res_table$inflow_cdec_latest_meandly_af_date)
...
cdec_res_table <- cdec_res_table %>%
                         mutate(
                           totalcapacity_cdec_NA_instant_af_date = as.character(totalcapacity_cdec_NA_instant_af_date), 
                           ...
                           histstorvol_cdec_NA_histmean_af_date = as.character(histstorvol_cdec_NA_histmean_af_date),
                           histstorval_cdec_NA_histmean_percent_date = as.character(histstorval_cdec_NA_histmean_percent_date),
                           outflow_cdec_latest_meandly_af_date = as.character(outflow_cdec_latest_meandly_af_date),
                           inflow_cdec_latest_meandly_af_date = as.character(inflow_cdec_latest_meandly_af_date),
...
cdec_res_table <- cdec_res_table %>%
                         mutate(
                           totalcapacity_cdec_NA_instant_af_date = as.double(totalcapacity_cdec_NA_instant_af_date), 
                           ...
                           histstorvol_cdec_NA_histmean_af_date = as.double(histstorvol_cdec_NA_histmean_af_date),
                           histstorval_cdec_NA_histmean_percent_date = as.double(histstorval_cdec_NA_histmean_percent_date),
                           outflow_cdec_latest_meandly_af_date = as.double(outflow_cdec_latest_meandly_af_date),
                           inflow_cdec_latest_meandly_af_date = as.double(inflow_cdec_latest_meandly_af_date),
...
cdec_res_table <- cdec_res_table %>% mutate(date = ifelse(param == "totalcapacity", NA, date), date = as.Date(date))
as_tibble(cdec_res_table)  
...
cdec_res_table_dlychange <- right_join(cdec_res_table_latest, cdec_res_table_dlychange, by = c("res", "cdecid", "date")) %>%
                            mutate(value = value.y/(value.x - value.y) * 100 ) %>%
                            mutate(unit = "percentchange") %>%
                            mutate(timeseq = "dailychange") %>%
                            select(-value.x, -value.y)
as_tibble(cdec_res_table_dlychange)

cdec_res_table <- rbind(cdec_res_table, cdec_res_table_dlychange ) %>% select(-source) %>% rename_all(paste0, "_cdec") %>% 
