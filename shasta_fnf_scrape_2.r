...
shasta_fnf_percap_nextlatest_upstr_valchange <- shasta_fnf_percap_nextlatest_upstr %>%
                                              mutate(value = shasta_fnf_percap_nextlatest_upstr_valchange, timeseq = "dailychange") %>% mutate(date = date_mostrecent)
## rbind
...
shasta_fnf_percentempty <- shasta_fnf %>% filter(unit == "percentfull") %>% mutate(value = 100 - value, unit = "percentempty") %>% mutate(date = date_mostrecent)
...
shasta_remainingcap_latest <- left_join(shasta_fnf_stor_latest, shasta_fnf_totcap, by = "nwsid" , "param") %>% 
                   mutate(value = value.y - value.x, param = ifelse(param == "upstrstorage", "remainingupstrcapacity", "remainingcapacity")) %>% 
                   select(-value.x, -value.y, -param.x, -param.y)  
...
shasta_fnf_inflowdlyaf <- shasta_fnf %>% filter(param == "inflow", meastype == "meandly", unit == "cfs") %>% mutate(value = value * 1.98347, unit = "af")
shasta_fnf <- rbind(shasta_fnf, shasta_fnf_inflowdlyaf) 





shasta_fnf <- data.frame(shasta_fnf) %>% mutate(value = unlist(value)) %>% filter(timeseq != "nextlatest") %>% select(-source) %>% 
                                         rename_all(paste0, "_usbr") %>% 
